...
public class ShowConstantUse_debug extends GhidraScript {
    ...
    public void run() throws Exception {
        ...
        try {
            ...
            decomplib = setUpDecompiler(currentProgram);//设置反编译
            ...
            if (var != null && !var.isConstant()) {//非常量
                ...
                if (currentLocation instanceof FunctionParameterFieldLocation) {//函数参数
                ...
                else if (currentLocation instanceof VariableNameFieldLocation) {//变量名
                ...
                else if (currentLocation instanceof DecompilerLocation) {//反编译窗口
                    ...
                    if (token instanceof ClangVariableToken) {
                        ...
                        if (parent instanceof ClangStatement) {//变量所在参数为clang指令
                            ClangStatement clangStmt = (ClangStatement) parent;
                            PcodeOp pcodeOp = clangStmt.getPcodeOp();//获取对应的pcode
                            if (pcodeOp.getOpcode() == PcodeOp.CALL) {//call指令
                                Varnode input = pcodeOp.getInput(0);//调用函数的地址
                                if (input.isAddress()) {
                                    Address faddr = input.getAddress();
                                    Function f = getReferencedFunction(faddr);//找到地址所在的函数，引用该地址的函数
                                    int paramIndex = 0;
                                    for (int i = 0; i < clangStmt.numChildren(); i++) {
                                        ClangNode child = clangStmt.Child(i);
                                        if (child.equals(clangVar)) {//找到statment中对应的变量，确认是call指令的第几个参数
    ...
    private Function getReferencedFunction(Address functionAddress) {
        Function f = currentProgram.getFunctionManager().getFunctionAt(functionAddress);//通过地址获取函数
        // couldn't find the function, see if there is an external ref there.
        if (f == null) {//找不到函数
            Reference[] referencesFrom =
                    currentProgram.getReferenceManager().getReferencesFrom(functionAddress);////获取对一个地址的所有引用
    ...
    private Varnode getVarnodeLocation() {
        ...
        if (currentLocation instanceof DecompilerLocation) {//光标在反编译窗口
            DecompilerLocation dloc;

            // get the Varnode under the cursor
            dloc = (DecompilerLocation) currentLocation;
            ClangToken tokenAtCursor = dloc.getToken();//从gui的光标未知获取值
            var = DecompilerUtils.getVarnodeRef(tokenAtCursor);//获取变量的varnode
            ...
            HighSymbol highVar = computeVariableLocation(currentProgram, currentLocation);//通过光标获取变量
            if (highVar != null) {
     ...
     *  在已反编译函数中给定varnode的情况下，回溯到常量这对于无法反编译的函数没有用处
     * @param var
    ...
    private HashMap<Address, Long> backtrackParamToConstant(Function f, int paramIndex,
        ...
        pvnode = parm.getFirstStorageVarnode();//获得变量存储的varnode
        ...
        if (def == null) {
            ...
            if (hvar instanceof HighParam) {
                funcList.add(new FunctionParamUse(function.getName(), function.getEntryPoint(),
                        ((HighParam) hvar).getSlot(), hvar.getRepresentative(), defUseList));//添加函数信息
                return;
            }
